...
class GaugeUzawaTwoPhaseFlowSolver:
    ...
    def __init__(self, pde, mesh:TriangleMesh, up=2, pp=1, phip=1, dt=0.01, q=4):
        self.pde = pde
        self.mesh = mesh
        self.dt = dt
        self.q = q
        self.time = 0.0
        self.up = up  # Polynomial degree for velocity space
        self.pp = pp
        self.phip = phip
    ...
    def solve_momentum(self, t1=0.0, phi0=None, uh0=None, phi=None, s=None):
        ...
        @cartesian
        def velocity_solution(p):
            return self.pde.velocity_solution(p, t1)
        BC = DirichletBC(self.uspace, velocity_solution, threshold=None, method='interp')
        A, b = BC.apply(A, b)
    ...
    def solve_pressure_correction(self, phi=None, us=None):
        ...
        print(bm.sum(bm.abs(us)))
        print(bm.sum(bm.abs(b)))
        # Solve
        p_correction = spsolve(A, b, solver='scipy')
        print(bm.sum(bm.abs(p_correction)))
        return p_correction 
    ...
    def update_velocity(self, phi=None, us=None, ps=None):
        ...
        @barycentric
        def rho_ps(bcs):
            return (1/self.density(phi)(bcs)[..., None]) * ps.grad_value(bcs)
        muti_idx = bm.multi_index_matrix(self.up,dim = 2)/self.up
        local_rho_ps = rho_ps(muti_idx) # cqd
        local_rho_ps = bm.permute_dims(local_rho_ps, (0,2 ,1))  # (qd, c)
        cell2dof = uspace.cell_to_dof()

        dof_num = bm.zeros(uspace.number_of_global_dofs(), dtype=bm.int32)
        dof_num = bm.index_add(dof_num, cell2dof , bm.ones_like(cell2dof))
        dof_rho_ps = bm.zeros((uspace.number_of_global_dofs()), dtype=bm.float64)
        dof_rho_ps = bm.index_add(dof_rho_ps, cell2dof, local_rho_ps.reshape(-1,cell2dof.shape[1]))
        dof_rho_ps /= dof_num  # average over cells
    ...
    def update_guage(self, s0=None, us=None):
        ...
        @barycentric
        def source_coef(bcs):
            result = s0(bcs) - bm.einsum('cqii->cq', us.grad_value(bcs))
            return result
        bcs = self.bcs
        ws = self.ws
        cm = self.mesh.entity_measure("cell")
        local_s = bm.einsum('q,cq->c',ws,source_coef(bcs))*cm
        cell2dof = self.pspace.cell_to_dof()
        dof_s = bm.zeros(self.pspace.number_of_global_dofs(), dtype=bm.float64)
        dof_s = bm.index_add(dof_s, cell2dof, local_s[:,None])
        sm = bm.zeros(self.mesh.number_of_nodes(), dtype=bm.float64)
        sm = bm.index_add(sm, cell2dof, cm[:,None])
        dof_s /= sm
    ...
    def time_step(self):
        ...
        self.phi[:] = self.solve_phase_field(phi0=self.phi0, uh0=self.uh0, t1=self.time)
        # Step 2: Solve momentum equation (prediction step)
        self.us[:] = self.solve_momentum(phi=self.phi, phi0=self.phi0, uh0=self.uh0, t1=self.time, s=self.s)
        # Step 3: Pressure correction
        self.ps[:] = self.solve_pressure_correction(phi=self.phi, us=self.us)
        ...
        self.s[:] = self.update_guage(s0=self.s, us=self.us)
    ...
    def get_error(self, t):
        ...
        p_error = self.mesh.error(self.ph, lambda p: self.pde.pressure_solution(p, t), power=2)
        
        # Phase-field error
        # phi_exact = self.phispace.interpolate(lambda p: self.pde.phase_solution(p, t))
        phi_error = self.mesh.error(self.phi,lambda p: self.pde.phase_solution(p, t), power=2)
